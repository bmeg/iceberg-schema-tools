...
def _bundle_schemas(output_path, base_uri):
    ...
    logger.info(f"Aggregated schema written to {output_path / pathlib.Path('aced-bmeg.json')}")
...
def _bundle_cytoscape(output_path, edges):
    """Write edge schemas to docs."""
    ...
    logger.info(f'Cytoscape edges written to {fn}')
    ...
    with open(fn, 'w', newline='') as tsv_file:
        ...
        for edge in edges.values():
            ...
            if edge['is_primary']:
                writer.writerow([edge['source_type'], edge['source_property_name'], edge['destination_type']])
    logger.info(f'Cytoscape primary edges written to {fn}')


def _gen3_cytoscape(output_path, cytoscape_output_path):
    with open(output_path / pathlib.Path("aced.json"), "r") as fp:
        gen3_schema = json.load(fp)

    fn = cytoscape_output_path / pathlib.Path('edges.sif.gen3.tsv')
    with open(fn, 'w', newline='') as tsv_file:
        writer = csv.writer(tsv_file, delimiter='\t', lineterminator='\n')
        writer.writerow(['sourceName', '(edgeType)', 'targetName'])

        for name, vertex in gen3_schema.items():
            if name.startswith("_"):
                continue
            for link in vertex['links']:
                writer.writerow([vertex['id'], link['name'], link['target_type']])
    logger.info(f"Gen3 cytoscape written to {fn}")


@cli.command('generate')
@click.option('--output_path', required=True,
              default='iceberg/schemas',
              show_default=True,
...
def bundle_schema(output_path, config_path, gen3_fixtures, cytoscape_output_path, generate_edge_schemas):
    ...
    classes, edges, incoming_edges = _find_dependencies(gen3_config)

    schemas = {}

    _extract_vertices(classes, gen3_config, incoming_edges, output_path, schemas)
    ...
    cytoscape_output_path = pathlib.Path(cytoscape_output_path)
    assert cytoscape_output_path.is_dir()
    _bundle_schemas(output_path, base_uri)

    _bundle_cytoscape(cytoscape_output_path, edge_schemas)

    _gen3_schemas(gen3_config, output_path, schemas, gen3_fixtures, edge_schemas)

    _gen3_cytoscape(output_path, cytoscape_output_path)


def _extract_vertices(classes, gen3_config, incoming_edges, output_path, schemas):
    ...
    logger.info(('Individual Vertex schemas written to', output_path))


def _find_dependencies(gen3_config):
    """expand dependencies"""
    ...
    return classes, edges, incoming_edges


@cli.command(name='publish')
@click.argument('dictionary_path', default='iceberg/schemas/aced.json')
                    ...
                    if enum_reference_type not in desired_vertices:
                        if v.field_info.description and 'Note:' not in v.field_info.description:
                            v.field_info.description += \
                                " Note: following not in scope, see config.dependency_order."
                        ...
                        logger.warning(f"{klass.__name__}.{k} {enum_reference_type} not in scope.")
                        continue
                    edge_info = EdgeInfo(**{
...
def _add_reference_backrefs(incoming_edges, schema, manually_curated_edges):
    ...
    for key, property_ in schema['properties'].items():
        ...
        edge_config, reference_backrefs = \
            _extract_reference_backrefs(incoming_edges, key, manually_curated_edges, property_, src_name)

        _log_warnings(edge_config, key, property_, reference_backrefs, schema)

        _ensure_targets(property_, reference_backrefs)
...
def _ensure_targets(property_, reference_backrefs):
    targets = []
    ...
    if len(targets) > 0:
        property_['targets'] = targets
    del property_['enum_reference_types']


def _log_warnings(edge_config, key, property_, reference_backrefs, schema):
    ...
    if len(reference_backrefs) == 0 and not edge_config and 'Resource' in property_['enum_reference_types']:
        logger.warning(('Any reference', f"{schema['title']}.{key}", property_['enum_reference_types']))
    if len(reference_backrefs) == 0 and not edge_config and 'Resource' not in property_['enum_reference_types']:
        logger.warning(('Out of scope reference', f"{schema['title']}.{key}", property_['enum_reference_types']))
    if len(reference_backrefs) > 1:
        logger.info(
            ('multiple reference_backrefs', f"{schema['title']}.{key}", reference_backrefs))


def _extract_reference_backrefs(incoming_edges, key, manually_curated_edges, property_, src_name):
    edge_config = next(iter([edge_config for edge_label, edge_config in manually_curated_edges.items() if
                             edge_label.startswith(f"{src_name}_{key}")]), None)
    reference_backrefs = {}
    for dst_name in property_['enum_reference_types']:
        ...
        for backref in incoming_edges[dst_name][src_name]['backrefs']:
            if check_property_name and backref.startswith(key):
                reference_backrefs[dst_name] = backref
            elif not check_property_name:
                reference_backrefs[dst_name] = backref
    return edge_config, reference_backrefs
...
def _gen3_schemas(gen3_config, output_path, schemas, gen3_fixtures, edge_schemas):
    ...
    _extract_linked_vertices(config_categories, edge_schemas, gen3_config, ignored_properties, needs_to_string, schemas)
...
def _extract_linked_vertices(config_categories, edge_schemas, gen3_config, ignored_properties, needs_to_string,
                             schemas):
    linked_vertices = set()
    for k, schema in schemas.items():
        ...
        _simplify_gen3_schema(ignored_properties, needs_to_string, schema, schemas)
...
def _simplify_gen3_schema(ignored_properties, needs_to_string, schema, schemas):
