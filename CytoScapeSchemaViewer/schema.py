...
def is_edge(name, schema) -> bool:
    ...
    if "enum_reference_types" not in str(schema) or name is None:
        return False
    
    return "backref" in str(schema)
...
def bundle_cytoscape(input_path, output_path):
    ...
    with open(fn, 'wb') as tsv_file:
        ...
        for elem in Main_nodes:
            #pass
            nodes.append({"data":{"id":elem}})
            #tsv_file.write(orjson.dumps({"data":{"id":elem, "properties":[]}}))
